...
export type Plot = { x: number; y: number; color: string };

export type ProductWithPlot = Product & { plot: Plot };
 ...
 * @param siteWidth
 */
export const balanceItemsGreedy = (
  state: Store,
  ...
  const flattenedPlots = buckets.reduce(
    (acc: ProductWithPlot[], bucket, index) => {
      let rollingLength = 0;
